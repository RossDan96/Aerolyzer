...
Express immediately takes 3rd place amongst these choices, because it's overly reliant on third party middleware. Rails and Django are similar in respect to included features, but Django has it's model API for increased ease of use. 
\subsection{Conclusion}
Django offers the most complete set of framework features while maintaining speed. Since Django is written in python and the Aerolyzer library is also written in python, integrating the Aerolyzer library would be easiest with Django.
...
\cite{SQLiteAbout}\cite{SQLiteUse}\cite{SQLiteFeat}
\subsubsection{MySQL}
\cite{MySQLDoc}
